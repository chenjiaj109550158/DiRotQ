...
## 2. 官方主表（零依賴配置，真 nunchaku kernel，五指標）——**28/30**

| 模型（步數/樣本） | 配置 | PSNR↑ | LPIPS↓ | SSIM↑ | FID-ref↓ | FID-GT↓ | 戰績 |
|---|---|---|---|---|---|---|---|
| **FLUX-schnell**（4/1000） | λ0.01 | **18.83** / 18.82 | **0.2309** / 0.2319 | 0.7426 / **0.7430** | **27.66** / 28.27 | **59.91** / 60.40 | **4:1** |
...
| **SANA-1.6B**（20/2500） | λ0.3+S_rms@0.25 | **19.86** / 19.76 | **0.1613** / 0.1624 | **0.7446** / 0.7426 | 10.62 / **10.41** | **27.18** / 27.22 | **4:1** |
...
2026-09-05 裁定 A：sana/schnell 換用本機乾淨重建版權重與其數字——
六 release 全數「本機從零可位級重現」；schnell SSIM 一格以 0.0004
（實測噪聲帶內）轉負、大表 29/30→28/30；舊版數字存 git 史。）
...
2. **λ 網格上端擴展**（PLAN_LAMBDAEXT）：turbo/dev 校準端 λ\*→1e6，
   ...
   → λ 極端存在校準→官方泛化縫隙。**裁定（2026-09-04，使用者）：A
   凍結原網格**——主表維持 29/30，λext 以消融呈現（原網格屬預註冊，
   事後擴展僅在受益模型採納有 cherry-picking 之嫌）。
