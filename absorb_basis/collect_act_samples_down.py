"""PLAN_NEXTQ P2: collect raw activation samples for the K=12288
down-projection inputs (mirrors collect_act_samples.py, hooks from
collect_cov_down.down_specs; single-block proj_out takes the MLP half
via col_slice).

Usage (svdquant env, repo root):
  python absorb_basis/collect_act_samples_down.py \
...
Output: {cov_key: X [cap, 12288] float16}
"""
...
from absorb_basis.collect_cov_down import down_specs
...
def main():
    ...
    ap.add_argument("--stride", type=int, default=1,
                    help="take every Nth cache file (dev: 5)")
    args = ap.parse_args()

    cache_files = sorted(glob.glob(os.path.join(args.calib_dir, "*.pt")))
    cache_files = cache_files[:: args.stride]
    assert cache_files, args.calib_dir
    print(f"{len(cache_files)} cache files (stride {args.stride})")
    ...
    def make_hook(key, col_slice):
        def hook(module, hargs):
            ...
            idx = torch.randint(0, x.shape[0], (rows_per_batch,),
        ...
        for k, v in batch_data[0]["input_kwargs"].items():
            if isinstance(v, torch.Tensor):
                ...
                if v.ndim >= 1 and v.shape[0] == 1:
                    kwargs[k] = torch.cat(
                        [x.to(model_dtype) if x.is_floating_point() else x
                         for x in vals], dim=0).to("cuda")
                else:
                    kwargs[k] = (v.to(model_dtype) if v.is_floating_point()
                                 else v).to("cuda")
            else:
                kwargs[k] = v
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        for _ in range(2):
            try:
                ...
                break
        for _ in tqdm(range(len(file_batches)), desc="down-act", dynamic_ncols=True):
            try:
                io_queue.append(pool.submit(load_batch, next(it)))
            except StopIteration:
                pass
    ...
    for h in hooks:
        h.remove()
    out = {k: torch.cat(v, dim=0)[: args.cap] for k, v in samples.items() if v}
    os.makedirs(os.path.dirname(args.out), exist_ok=True)
    torch.save(out, args.out)
    print(f"saved {len(out)} down sample tensors -> {args.out}")
